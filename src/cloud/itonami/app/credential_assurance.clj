(ns cloud.itonami.app.credential-assurance
  ...
  Everything here is PURE. It grades a stored map. The ceremony lives in
  `cloud.itonami.app.passkey`, and the enforcement in
  `cloud.itonami.app.authority`, so this rule can be tested without a browser,
  a device, or a human finger."
  (:require [clojure.string :as str]))

(def schema "cloud.itonami.app.credential-assurance.v1")
...
(defn policy-for
  ...
  (merge default-policy
         (get-in configuration [:authorities authority-key :credential-policy])))
