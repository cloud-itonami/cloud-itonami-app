(ns cloud.itonami.app.kotobase-federation
  ...
  Kotobase.  This module therefore does not pretend that the user signed a
  CACAO: after a real local passkey session, this deployment signs a short,
  one-use assertion that names the user's P-256 did:key as its subject."
...
(defn mint-assertion
  "Mint a two-minute assertion after a *human passkey* session.

  Email and agent sessions are intentionally rejected.  They may use the app,
  but neither proves that a passkey ceremony rooted this federation hand-off."
  ...
  ([session now]
   ...
   (let [subject-did (session-subject-did session)]
     ...
     (let [iat (Instant/ofEpochSecond (.getEpochSecond ^Instant now))
           ...
           minted (cacao/mint {:seed (issuer-seed)
                               :aud audience
                               :iat (str iat)
                               :exp (str exp)
