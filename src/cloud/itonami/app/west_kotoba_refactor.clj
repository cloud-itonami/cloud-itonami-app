...
(defn scan
  ...
  (let [summaries (->> (projects root)
                       (keep (fn [{:keys [name checkout] :as project}]
                               (try
                                 (let [result (inspect-resolved project {:limit 1})
                                       total (:candidate-count result)]
                                   (when (pos? total)
                                     {:name name
                                      :path (get-in result [:project :path])
                                      :checkout checkout
                                      :clj-files total
                                      :kotoba-files (get-in result [:counts :kotoba])
                                      :first-candidate (get-in result [:candidates 0 :path])}))
