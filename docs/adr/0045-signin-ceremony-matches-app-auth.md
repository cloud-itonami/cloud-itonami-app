...
2. **Device-local Passkey, Email, and this-device register are recovery.** They live in a closed DADS accordion. Provider SSO is not exposed by the default or `itonami` profile; its backend is compatibility-only and requires explicit deployment opt-in. An interrupted owner ceremony (`passkey-required?`) opens the recovery accordion. Organization invite stays a separate local flow — it is not hosted identity.
