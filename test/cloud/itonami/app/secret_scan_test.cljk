...
(def ^:private dashes
  "The PEM envelope is assembled from parts rather than written out.

  This file is scanned by the very gate it tests, so a contiguous PEM header
  in this source IS a finding. Measured: the first version of this test wrote
  the header as a literal and made `secret-scan` fail on its own commit —
  which `the-repositorys-own-history-is-clean` below then caught, so the pair
  worked even though the author had not re-run it after committing."
  (apply str (repeat 5 "-")))
...
(defn- pem-shaped-key []
  (let [random (java.security.SecureRandom.)
    ...
    (str dashes "BEGIN RSA PRIVATE KEY" dashes "\n"
         (str/join "\n" (repeatedly 6 line))
