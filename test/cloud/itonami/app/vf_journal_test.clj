...
(defn- event
  ...
  event_test fixtures: `produce` increments both registers, so the event
  must name the resource it creates (`:resource-inventoried-as`) and carry
  a resource quantity — `bump` with a nil resource id is exactly the
  :missing-resource fold error this slice must not paper over."
  [over]
...
(deftest hand-edited-root-refuses-to-load
  (with-fresh-journal
   (fn []
     ...
     ;; The org name flows through org-journal-file unchanged (`:org-test`
     ;; has no characters the sanitizer replaces), so the file is
     ;; "org-test.vf-journal.edn" — spelled from the same `org` symbol the
     ;; appends used, never a second copy of the name (measured 2026-09-03:
     ;; a stale "org_test" spelling made the test read a file that was
     ;; never written, and the tamper proof 'passed' on the wrong file).
