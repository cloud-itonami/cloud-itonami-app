...
**falsify-104 (2026-09-09, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→104 の 20 時点観測 / 19 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-103 指定の閾値「18 反復超の連続持続」を充足 (19 > 18) し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 122 → 116 Gi (約 -6 Gi)、annex available 130.49 → 124.99 GB (約 -5.50 GB) と falsify-103 の 122 Gi からの回復は認められず継続低下** — 主張 (c)「falsify-101/103 の低位が一過性で 148-163 Gi 帯へ回復する」仮説は本反復で成立せず (REFUTED)、falsify-101 の -29 Gi ステップダウンは一過性でなく小幅 (-5〜-6 Gi) に減速したまま継続低下中と確定 (100 Gi 未満の緊急フラグ未到達)。local annex keys 786 / 11.68 GB 不変ゆえ低下は annex 外部 (TCC 遮断で特定不能、operator の du 調査を異常系・優先度高で Tier 2 継続提出)。増加源 PID 70643 不変のまま (falsify-85..103 と同一 PID、export_and_sync 不在、ps の自助マッチ行は測定ハーネス自身を除外)。score 3 のまま (0 ベース恒久化傾向の更なる精緻化、実緑測定でのみ上昇する規約、avail 継続低下は risk-5 / structural-decline-monitor の経過観測継続として記録)。次の 1 アクション: 同 vector + keys + avail を再実測し **19 反復超の連続持続** or 再蓄積再開を判別、avail 116 Gi の更なる継続低下 (100 Gi 未満で緊急フラグ) vs 減速したままの低位定着 (116 Gi 前後) vs 回復 (148-163 Gi 帯、structural-decline-monitor 再評価) を観測。附帯 (実装軸): origin/main は **b1b4b85f のまま** (falsify-102/103 と同一、新規前進なし)。(evidence/2026-09-09-falsify-104.md)
**falsify-105 (2026-09-09, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→105 の 21 時点観測 / 20 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-104 指定の閾値「19 反復超の連続持続」を充足 (20 > 19) し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 116 → 113 Gi (約 -3 Gi)、annex available 124.99 → 121.58 GB (約 -3.41 GB) と falsify-104 の 116 Gi からの回復は認められず 3 反復連続の継続低下** — 主張 (c)「falsify-101/103/104 の低位が一過性で 148-163 Gi 帯へ回復する」仮説は本反復で成立せず (REFUTED)、falsify-101 の -29 Gi ステップダウンは一過性でなく小幅 (-5→-6→-3 Gi) に減速したまま継続低下中と確定 (100 Gi 未満の緊急フラグ未到達)。local annex keys 786 / 11.68 GB 不変ゆえ低下は annex 外部 (TCC 遮断で特定不能、operator の du 調査を異常系・優先度高で Tier 2 継続提出)。増加源 PID 70643 不変のまま (falsify-85..104 と同一 PID、export_and_sync 不在、測定ハーネス自身の ps 自助マッチ行は grep -v grep で除外)。score 3 のまま (0 ベース恒久化傾向の更なる精緻化、実緑測定でのみ上昇する規約、avail 継続低下は risk-5 / structural-decline-monitor の経過観測継続として記録)。次の 1 アクション: 同 vector + keys + avail を再実測し **20 反復超の連続持続** or 再蓄積再開を判別、avail 113 Gi の更なる継続低下 (100 Gi 未満で緊急フラグ) vs 減速したままの低位定着 (113 Gi 前後) vs 回復 (148-163 Gi 帯、structural-decline-monitor 再評価) を観測。附帯 (実装軸): origin/main は **b1b4b85f のまま** (falsify-102/103/104 と同一、3 反復連続 no-advance)。(evidence/2026-09-09-falsify-105.md)
**falsify-106 (2026-09-09, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→106 の 22 時点観測 / 21 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-105 指定の閾値「20 反復超の連続持続」を充足 (21 > 20) し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 113 → 114 Gi (116081 → 116596 MB, 約 +0.5 Gi)、annex available 121.58 → 122.16 GB (約 +0.58 GB)** — falsify-103/104/105 の -5→-6→-3 Gi 継続低下は本反復で**停止し微増に転換**、4 反復連続低下後の低位 (113-114 Gi) 定着シグナルを初観測 — 主張 (c)「148-163 Gi 帯への回復」は本反復でも成立せず (REFUTED) が、継続低下の記述を「低下停止・微増転換」へ範囲修正、structural-decline-monitor を「低下停止の持続性観察」モードへ更新 (100 Gi 未満で緊急フラグ据え置き)。local annex keys 786 / 11.68 GB 不変ゆえ過去の低下は annex 外部 (TCC 遮断で特定不能、operator の du 調査を異常系・優先度高で Tier 2 継続提出)。増加源 PID 70643 不変のまま (falsify-85..105 と同一 PID、export_and_sync 不在、ps 自助マッチ行は grep -v grep で除外)。附帯: 本反復の `git ls-remote origin main` は remote 不在 fatal で失敗、origin/main 前進確認は保留 (前反復 falsify-105 時点 b1b4b85f、4 反復連続 no-advance、本体 checkout porcelain 0 行 = clean)。score 3 のまま (0 ベース恒久化傾向の更なる精緻化、実緑測定でのみ上昇する規約、avail 低下停止は risk-5 / structural-decline-monitor 観測モード更新として記録)。次の 1 アクション: 同 vector + keys + avail を再実測し **21 反復超の連続持続** or 再蓄積再開を判別、avail 114 Gi の低下停止 (微増) 持続 (113-114 Gi 帯定着) vs 低下再開 (100 Gi 未満で緊急フラグ) vs 回復 (148-163 Gi 帯) を観測。加えて `git remote -v` で origin 失踪の原因特定と origin/main 前進確認の復旧。(evidence/2026-09-09-falsify-106.md)
**falsify-107 (2026-09-09, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→107 の 23 時点観測 / 22 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-106 指定の閾値「21 反復超の連続持続」を充足 (22 > 21) し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 113.9 Gi → 35.7-36.3 Gi (36580-37158 MB、反復内複数時点)、annex available 122.16 → 38.26-38.86 GB と約 -78 Gi / -83 GB の史上最大単反復ステップダウン**を観測し **100 Gi 未満緊急フラグに到達** — 主張 (c)「falsify-106 の低下停止・113-114 Gi 帯定着が持続する」仮説は帯外逸脱で成立せず (REFUTED)、falsify-101 の -29 Gi を大幅に上回る減退。短期 (5-20 秒間隔) df でも 36.0→35.7 Gi と微減継続。local annex keys 786 / 11.68 GB 不変ゆえ減退は annex 外部 (TCC 遮断で特定不能、operator の du 調査を異常系・**最優先**で Tier 2 再提出)。増加源 PID 70643 不変のまま (export_and_sync 不在、ps 自助マッチ行は grep -v grep で除外)。**origin 失踪を解明**: dns-resolver repo の `git remote -v` は origin 無し、実在は `cloud-itonami` + `kotobase` のみ (remote 名変更/再構成が原因、ネットワーク障害ではない)、`git ls-remote cloud-itonami main` は成功し 566928f (測定 HEAD と同一)。附帯 (実装軸): 本体 app の origin/main は **f541135f へ前進** (b1b4b85f から更新、falsify-102..105 の 4 反復連続 no-advance 解消、ls-remote のみで本体 touch なし)。score 3 のまま (0 ベース恒久化傾向の更なる精緻化、実緑測定でのみ上昇する規約、avail 緊急フラグ到達は risk-5 / structural-decline-monitor 緊急観測モード移行として記録)。次の 1 アクション: 同 vector + keys + avail を再実測し **23 反復超の連続持続** or 再蓄積再開を判別、avail 36 Gi 帯の更なる低下 vs 定着 vs 回復を観測。加えて (1) 本体 origin/main f541135f 追従は kanban/human へ透過 (本体は他 bot WIP 触れず)、(2) dns-resolver remote 名変更 (origin→cloud-itonami) の意図を operator 確認事項として Tier 2 記録、(3) avail 緊急フラグ下の operator du 調査を最優先 Tier 2 として再提出。(evidence/2026-09-09-falsify-107.md)
**falsify-108 (2026-09-09, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→108 の 24 時点観測 / 23 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず (SURVIVED)。avail は **df 実測 35304 MB ≒ 34.5 Gi (反復内 3 時点 ±1 MB 実質静止)、annex info available 36.92 GB** で falsify-107 (35.7-36.3 Gi / 38.3-38.9 GB) 比約 -1.5 Gi/GB の微減、100 Gi 未満緊急フラグは継続 — 主張 (c)「36 Gi 帯から回復 (or 大幅低下)」は成立せず (REFUTED)、Gi 十台のステップダウンはなく **34-36 Gi 帯低位定着シグナルを初観測** (falsify-106 の定着シグナルが 1 反復で破れた前例ゆえ 2 反復連続を持って初めて定着と判定、structural-decline-monitor は緊急観測モード継続 + 低位帯定着の持続性観察を併記)。減退は annex 外部 (keys 786 / 11.68 GB 不変、operator の du 調査を最優先 Tier 2 継続)。増加源 PID 70643 不変 (export_and_sync 不在、ps 自助マッチ行は grep -v grep で除外)。附帯 (実装軸): 本体 origin/main は **f541135f のまま** (falsify-107 と同一、ls-remote のみ・本体 touch なし、本体 HEAD b1b4b85 clean)。表記ゆれ透過記録: falsify-107 の指定閾値は「23 反復超」だが既存パターンでは「22 反復超」の意と判断し 23 連続で充足、次反復閾値は「24 反復超」。score 3 のまま。次の 1 アクション: 同 vector + keys + avail を再実測し **24 反復超の連続持続** or 再蓄積再開を判別、avail 34-36 Gi 帯低位定着の持続 (2 反復連続で定着確定) vs 更なる低下 vs 回復を観測。(evidence/2026-09-09-falsify-108.md)
**falsify-109 (2026-09-09, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→109 の 25 時点観測 / 24 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず (SURVIVED)。avail は **df 実測 35504 MB ≒ 34.7 Gi (反復内 3 時点 ±0 MB 完全静止)、annex info available 37.13 GB** で falsify-108 (35304 MB ≒ 34.5 Gi / 36.92 GB) 比約 +0.2 Gi/GB の帯内微増、100 Gi 未満緊急フラグは継続 — 主張 (c)「34-36 Gi 帯からの逸脱 (大幅低下 or 回復)」は成立せず (REFUTED)、**34-36 Gi 帯の低位定着が 2 反復連続で確定** (falsify-108 の規約に従い確定判定、structural-decline-monitor は緊急観測モード維持 + 観察対象を低位定着の持続性 3 反復目へ更新)。減退は annex 外部 (keys 786 / 11.68 GB 不変、operator の du 調査を最優先 Tier 2 継続)。増加源 PID 70643 不変 (export_and_sync 不在、ps 自助マッチ行は grep -v grep で除外)。附帯 (実装軸): 本体 origin/main は **98bec5dc へ前進** (f541135f から更新、PR #303 merge 相当、falsify-107/108 の 2 反復 no-advance 解消、本体 checkout HEAD も 98bec5d に更新・clean、本 bot は ls-remote/rev-parse 読み取りのみで本体 touch なし)。score 3 のまま。次の 1 アクション: 同 vector + keys + avail を再実測し **25 反復超の連続持続** or 再蓄積再開を判別、avail 34-36 Gi 帯低位定着の持続 (3 反復目) vs 帯外逸脱を観測。(evidence/2026-09-09-falsify-109.md)
55a9e40 と同形式: maturity: falsify-110 -- operations/retention axis -- 0-base persists 16th observation/15th consecutive iteration (0 regular unused all vectors / keys 786 static) SURVIVED; avail 34-36 Gi band-hold theory REFUTED at lower edge (df 35504 -> 34789 MB ~34.0 Gi, -0.7 Gi, annex 37.13 -> 36.38 GB) re-anchored to ~34 Gi low band with +/-1 Gi wobble, <100 Gi emergency flag CONTINUED; incidental: app origin/main f541135f unchanged 3rd iteration, app HEAD 98bec5dc clean; score 3 unchanged (evidence/2026-09-09-falsify-110.md)

**falsify-111 (2026-09-11, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→111 の 17 時点観測 / 16 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-110 指定の閾値「16 反復超の連続持続」を充足し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 34789 → 193392 MB ≒ 34.0 → 188.9 Gi、annex available 36.38 → 203.15 GB と約 +154.9 Gi / +166.8 GB の帯外上方回復 (大幅ジャンプ)** を観測 — 主張 (c)「34 Gi 前後 ±1 Gi ブレ帯が持続する」仮説は帯外逸脱で成立せず (REFUTED)、逸脱方向は falsify-107 の -78 Gi 急落と対称の上方回復。local annex keys 786 / 11.68 GB 不変ゆえ回復は annex 外部 (外部解放源、TCC 遮断で特定不能、operator の du 調査を Tier 2 継続)。**緊急フラグ (<100 Gi) は解除**、structural-decline-monitor は緊急観測モード解除。帯アンカー定着判定枠組みは 3 反続で 3 回破れたため、「単時点実測値の忠実記録 + ±10 Gi 超/反復の大幅変動検知」へ簡素化 (範囲修正)。増加源 PID は falsify-85..110 と同一とされてきた 70643 から **62791 へ変化** (resident 再起動、Wed03PM 起動、export_and_sync 不在、ps 自助マッチ行は grep -v grep で除外)。附帯 (実装軸): 本体 origin/main は **49d0f085 へ前進** (f541135f から更新、PR #330 merge 相当、3 反復連続 no-advance 解消、本体 HEAD 同一 49d0f085・clean、本 bot は rev-parse/ls-remote origin 読み取りのみで本体 touch なし; 本体 repo の remote は origin のみで dns-resolver 側の remote 名変更は波及なし)。score 3 のまま。次の 1 アクション: 同 vector + keys + avail を再実測し **17 反復超の連続持続** or 再蓄積再開を判別、avail 188.9 Gi 帯の維持 vs 再急落 (±10 Gi 級ステップ変動再検出時は structural-decline-monitor 再強化) を観測。(evidence/2026-09-11-falsify-111.md)

**falsify-112 (2026-09-11, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→112 の 18 時点観測 / 17 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-111 指定の閾値「17 反復超の連続持続」を充足し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 193392 → 183066/183048 MB ≒ 188.9 → 178.8 Gi、annex available 203.15 → 191.86 GB と約 -10.1 Gi / -11.3 GB の低下** — falsify-111 で簡素化した「±10 Gi 超/反復の大幅変動検知」が**初適用反復で発火** (低下方向、主張 c 成立)。ただし逸脱幅は falsify-107 (-78 Gi) / falsify-111 (+154.9 Gi) より 1 桁小さく外部源起因の揺れ範囲内、keys 786 / 11.68 GB 不変ゆえ annex 外部。緊急フラグ (<100 Gi) 非該当、帯アンカー推定は行わない (falsify-111 の範囲修正を継承)。**増加源 PID が再度変化**: falsify-111 実測の 62791 は消滅、実測は **PID 61573** (itonami-app-resident、12:42PM 起動、反復内 2 時点不変) — 70643 → 62791 → 61573 の短期 2 度再起動は新規観測事実。附帯 (実装軸): 本体 origin/main は **5b2ae3de へ前進** (49d0f085 から更新、cljk rename merge 相当、本体 HEAD 同一・clean、読み取りのみ)。score 3 のまま。次の 1 アクション: 同 vector + keys + avail を再実測し **18 反復超の連続持続** or 再蓄積再開を判別、avail 178.8 Gi 帯の維持 vs ±10 Gi 級ステップ変動の継続/再発を観測。(evidence/2026-09-11-falsify-112.md)

**falsify-113 (2026-09-11, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→113 の 19 時点観測 / 18 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-112 指定の閾値「18 反復超の連続持続」を充足し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 183066/183048 → 172181 MB ≒ 178.8 → 168.1 Gi、annex available 191.86 → 180.45 GB と約 -10.7 Gi / -11.4 GB の低下** — falsify-112 に続く **±10 Gi 級ステップ変動検知の 2 反復連続発火** (低下方向 -10.1 → -10.7 Gi、ほぼ同規模の単調系列) — 主張 (c) 成立。2 反復連続の同方向・同規模低下は falsify-111 の「外部源起因の揺れ範囲内」単発解釈では説明が弱く、structural-decline-monitor を「±10 Gi 級低下の継続観察」モードへ更新 (3 反復連続で構造的低下と確定判定、緊急フラグ <100 Gi 非該当)。keys 786 / 11.68 GB 不変ゆえ低下は annex 外部 (operator の du 調査を Tier 2 継続)。観測系列 113.9 → 34.5 → 34.7 → 34.0 → 188.9 → 178.8 → 168.1 Gi。**増加源 (resident) の実測不在は新規事実**: falsify-112 実測の PID 61573 は消滅、`ps aux | grep itonami-app-resident | grep -v grep` は **ヒット 0 行 (rc=1)** — 70643 → 62791 → 61573 → (不在) の系列は「常駐が常態」の前提を揺るがす観測。avail 低下と resident 停止が同時点に観測されたことは相関候補として記録 (因果特定不能、TCC 遮断)。附帯 (実装軸): 本体 origin/main は **33037469 へ前進** (5b2ae3de から更新、kbb cutover merge 相当、本体 HEAD 同一・clean、読み取りのみ)。附帯: dns-resolver HEAD は fd9aa865 → **c8090649 へ前進** (要因の実行者は operator 確認事項 Tier 2)。score 3 のまま。次の 1 アクション: 同 vector + keys + avail を再実測し **19 反復超の連続持続** or 再蓄積再開を判別、avail の -10 Gi 級低下が 3 反復連続 (構造的低下確定) か否か、resident 不在の継続 (ps 併走 export_and_sync、自助マッチ行 grep -v grep 除外) を観測。(evidence/2026-09-11-falsify-113.md)

**falsify-114 (2026-09-12, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→114 の 20 時点観測 / 19 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-113 指定の閾値「19 反復超の連続持続」を充足し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 172181 → 159426 MB ≒ 168.1 → 155.7 Gi、annex available 180.45 → 167.07 GB と約 -12.4 Gi / -13.4 GB の低下** — falsify-112/113 に続く **±10 Gi 級ステップ変動検知の 3 反復連続発火** (低下方向 -10.1 → -10.7 → -12.4 Gi、単調系列で幅が拡大) — 主張 (c) 成立、**falsify-113 の判定規約「3 反復連続で構造的低下と確定」が発動し構造的低下を確定** (緊急フラグ <100 Gi 非該当、余裕約 55.7 Gi)。keys 786 / 11.68 GB 不変ゆえ低下は annex 外部 (operator の du 調査を Tier 2 継続、緊急フラグ到達予測時点の算出を追加)。観測系列 113.9 → 34.5 → 34.7 → 34.0 → 188.9 → 178.8 → 168.1 → 155.7 Gi。**resident は新 PID 62828 (3:15PM 起動、nbb 経由 itonami-app-resident.cljs) で再出現** — falsify-113 の実測不在は解消、70643 → 62791 → 61573 → 不在 → 62828 の 4 回状態遷移は「単一常駐」ではなく頻繁再起動する監視対象という新規事実 (起動管理主体は operator 確認事項 Tier 2)。附帯 (実装軸): 本体 origin/main は **33037469 のまま** (falsify-113 と同一、本体 HEAD 同一・clean、読み取りのみ)。score 3 のまま。次の 1 アクション: 同 vector + keys + avail を再実測し **20 反復超の連続持続** or 再蓄積再開を判別、avail の -10 Gi 級低下 4 反復目の有無 (継続なら 1 反復 -10 Gi 超ペース確定 + 緊急フラグ到達予測時点を Tier 2 算出)、resident PID 62828 の存続を観測。(evidence/2026-09-12-falsify-114.md)

**falsify-115 (2026-09-12, 運用/滞留 軸)**: 3 ベクトル (main / 全 refs / resident-refspec) の正規 unused は全て 0 のまま (partial 14 chunk のみ)、local annex keys は 786 → 786 (増加 0, falsify-85→115 の 21 時点観測 / 20 反復連続維持) — 主張 (a) の再出現も (b) の再蓄積も検出されず、falsify-114 指定の閾値「20 反復超の連続持続」を充足し 0 ベース恒久化傾向を更に精緻化 (SURVIVED)。avail は **df 実測 159426 → 158160 MB ≒ 155.7 → 154.5 Gi、annex available 167.07 → 165.74 GB と約 -1.2 Gi / -1.3 GB の帯内微減** — 主張 (c)「±10 Gi 超/反復の 4 反復目発火」は本反復で発生せず (REFUTED)。falsify-114 で確定した「構造的低下」は低下幅が 1 桁縮小して継続する形に精緻化: 低下方向 4 反復連続 (-10.1 → -10.7 → -12.4 → -1.2 Gi) だが -10 Gi 級 → -1 Gi 級へ減速、1 反復 -10 Gi 超ペースは実勢と確定せず緊急フラグ到達予測時点の算出は未実施のまま記録 (ペース外挿の捏造をしない)。緊急フラグ (<100 Gi) 非該当 (154.5 Gi、余裕約 54.5 Gi)。keys 786 / 11.68 GB 不変ゆえ低下は annex 外部 (operator の du 調査を Tier 2 継続)。観測系列 113.9 → 34.5 → 34.7 → 34.0 → 188.9 → 178.8 → 168.1 → 155.7 → 154.5 Gi。**resident は 5 回目の状態遷移**: falsify-114 実測の PID 62828 (3:15PM 起動) は消滅、新 PID **66954** (11:21PM 起動、nbb 経由 itonami-app-resident.cljs) が実測在在 — 70643 → 62791 → 61573 → 不在 → 62828 → 66954 で「頻繁に再起動する監視対象」記述を支持 (起動管理主体は operator 確認事項 Tier 2 継続)。附帯 (実装軸): 本体 origin/main は **33037469 のまま** (falsify-113/114 から 3 反復連続 no-advance、ls-remote 実測、本体 HEAD 同一・porcelain clean、読み取りのみ)。score 3 のまま。次の 1 アクション: 同 vector + keys + avail を再実測し **21 反復超の連続持続** or 再蓄積再開を判別、avail の -1 Gi 級減速定着 vs -10 Gi 級再加速 (再加速なら緊急フラグ到達予測時点を Tier 2 算出)、resident PID 66954 の存続 (6 回目の状態遷移の有無) を観測。(evidence/2026-09-12-falsify-115.md)
| 運用 | 3 | falsify-116 (2026-09-17, 追記は falsify-117 反復で代行 — d3c2395 は evidence み commit で ledger 行が欠落していたのを実測で補正): avail 158160 → 55408 MB (≈54.1 Gi) の **-101.5 Gi step-change** で減速定着説を REFUTED、**緊急フラグ <100 Gi 初発動**。annex 測定ベクトル (app .git/annex, data/ledger) が No such file で遮断 — 0-base unused 系列は 21 時点で終了 (22 時点目は「測れなかった」)。resident 6 回目遷移 PID 12978。(evidence/2026-09-17-falsify-116.md) |
| 運用 | 3 | **falsify-117 (2026-09-18)**: 主張 (d)「786 keys 系列は app annex 消滅で消滅した」を **REFUTED** — find -maxdepth 5 で annex ディレクトリ 19 repo + superproject を列挙し、**dns-resolver .git/annex/objects が find -type f | wc -l 実測でちょうど 786**。新ベースライン: du 11136 MB ≈10.9 GiB、`git annex unused` 実測 unused object **0** / partially-transferred 一時ファイル 14 件 (~135 MB, MD5E-*.edn)。app annex 不在は再実測 (du rc=1)。df 48541 MB ≈47.4 Gi (95%) — 緊急フラグ <100 Gi **2 反続**、反復間 -6.7 Gi 追加低下で「ledger 消滅が容量を開放」相関候補は反対方向の証拠 (開放でなく追加消費)。因果は測定不能のため据え置き。附帯: app origin/main **fbdb91f** 実測 (falsify-116 未測定分を補完、33037469 から advance)、dns-resolver HEAD c8090649 不変、resident PID 12978 継続 (新規遷移なし)。Tier 2 報告: 緊急フラグ 2 反続 + dns-resolver dropunused 候補 14 件 ~135 MB (drop 判断は人間)。score 3 のまま (低下は測定で裏取れたが緑確認・修復は人間判断)。(evidence/2026-09-18-falsify-117.md) |
| 運用 | 3 | **falsify-118 (2026-09-18)**: 主張 (e)「avail の +32.8 Gi 級大回復は dns-resolver annex 系列に帰属」を **REFUTED** — 同一コマンド再実測で keys 786 不変 (find -type f)、unused object 0 / partial 一時ファイル 14 件 (同数)、du 11320 MB (前反復 11136 に対し +184 MB、うち tmp 179 MB) — 系列は開放 0、回復起点は annex 外部 (単一帰属は未確立、falsify-30/31 の教訓で「停止→回復」を充分条件としない)。avail 48541 → 81366 MB ≒ 47.4 → 79.5 Gi (91%)、**緊急フラグ <100 Gi 3 反続発動中**。avail 系列は -101.5 → -6.7 → +32.8 Gi の非単調振動。附帯: app origin/main **1ac45963** 実測 (fbdb91f から advance、直接 URL は Repository not found で superproject origin 経由のみ到達可)、dns-resolver HEAD c8090649 不変、resident PID 12978 継続 (新規遷移なし)。次の 1 アクション: 同測定再実測 + annex 外大容量 du (m365-archive 等) を附帯に加え帰属候補を絞る。score 3 のまま。(evidence/2026-09-18-falsify-118.md) |

| 運用 (falsify-119, 2026-09-18) | 3 | 主張 (f)「avail 非単調振動は m365-archive に由来 (falsify-18 帰属の現在形)」を REFUTED — m365-archive は home depth-1 の両候補地点 (~, ~/Documents) に不存在、du depth-1 top15 にも不在 (top は github 397651 MB ≒388 Gi)。dns-resolver annex は 3 反続完全静止 (786 keys / du 11320 MB / tmp 179 MB / unused 0 / partial 14)。df avail 78088 MB ≒76.3 Gi (92%)、falsify-118 比 -3.2 Gi、緊急フラグ <100 Gi 4 反続。resident PID 12978 継続、dns-resolver HEAD c8090649 不変、app origin/main は未測定 (不変とは主張しない)。score 3 維持 (evidence/2026-09-18-falsify-119.md) |
| 運用 | 3 | **falsify-120 (2026-09-18)**: 主張 (g)「緊急フラグ (<100 Gi) 5 反続」を REFUTED — df 実測 avail 78088→**125298 MB ≒ 122.3 Gi / 87%**、反復間 +47.2 Gi の大回復で 4 反続していたフラグは不発火。低位化系列は非単調 (-101.5 → -6.7 → +32.8 → -3.2 → +47.2 Gi) で単一帰属は未確立のまま (falsify-118 継続)。**正規測定点の移設を実測**: 旧 path `github/orgs/cloud-itonami/...` は不存在に変化、find 実測で `github/com-junkawasaki/orgs/cloud-itonami/cloud-itonami-dns-resolver` のみに再配置 — そこでの annex 再実測は **786 keys / 11320 MB / unused 0 / partial temp 14 件 / HEAD c8090649 不変**で 4 反続完全静止・path 変化後も生存。附帯: github depth-1 top は com-junkawasaki 258967 MB (~253 Gi) / wt-cs2 139865 MB (~136.6 Gi)、m365-archive は github 配下にも不在継続。resident は新 PID **1083** を実測 (12978 → 第 7 遷移)。score 3 不変 (evidence/2026-09-18-falsify-120.md) |
| 運用 | 3 | **falsify-121 (2026-09-19)**: 主張 (h)「partial 一時ファイル 14 件は系列静止の一部として持続」を REFUTED — 同 path 同一コマンド再実測で tmp/part count **0** (falsify-117..120 は 14 件で 3 反続同一)。keys 786 不変 (5 反続静止)、du 11403092 KB ≒ 10.9 GiB (帯内)、HEAD c8090649 不変。falsify-117 の Tier 2 提案「dropunused 候補 14 件 ~135 MB」は本反復で実在せず消滅済み (削除主体未測定)。附帯: df avail 108807940 KB ≒ **103.75 Gi / 89%**、falsify-120 比 -18.6 Gi で低下方向へ反転、緊急フラグ <100 Gi は不発火 2 反続 (閾値近傍に再接近、系列非単調: -101.5 → -6.7 → +32.8 → -3.2 → +47.2 → -18.6 Gi、単一帰属未確立継続)。resident PID 1083 継続。score 3 のまま (evidence/2026-09-19-falsify-121.md) |

| 運用 | 3 | **falsify-122 (2026-09-19)**: 主張 (i)「緊急フラグ (<100 Gi) 不発火の定着」を REFUTED — df 実測 avail 108807940→**73188864 KB ≒ 69.8 Gi / 93%**、falsify-121 比 **-30.9 Gi** で 2 反続の不発火を破り再発火。avail 系列は非単調継続: -101.5 → -6.7 → +32.8 → -3.2 → +47.2 → -18.6 → **-30.9 Gi**、単一帰属未確立 (falsify-118/119/120 継続)。主張 (j)「partial 一時ファイル 0 の定着」は SURVIVED — tmp/part 0 で 2 反続、keys 786 不変 (**6 反続静止**)、du 11403092 KB は falsify-121 と KB 完全同値、HEAD c8090649 不変 (6 反続)。resident PID 1083 継続。app origin/main 未測定。score 3 のまま (evidence/2026-09-19-falsify-122.md) |
| 運用 | 3 | **falsify-123 (2026-09-20)**: 主張 (k)「falsify-121/122 の partial 一時ファイル 0 は実在 0 を反映」を **REFUTED** — `git annex unused` が **14 件の partially transferred を再列挙** (rc=0)、`ls .git/annex/tmp` で 14 ファイル実在 (mtime すべて Sep 6、falsify-117 測定時点から不変)。falsify-121/122 の `find \( -name '*.tmp' -o -name '*.part' \)` は `.edn` 拡張子に不一致の **false negative** — 「dropunused 候補消滅・削除主体未測定」説は撤回、partial 14 件は falsify-117..123 を通じて連続実在、falsify-117 の Tier 2 提案 (dropunused 14 件、ls total 365232 blocks ≈ 約 174 MiB) を再提出。附帯: df avail **65652648 KB ≒ 62.6 GiB / 94%**、falsify-122 比 -7.2 Gi、緊急フラグ (<100 Gi) 2 反続発火、系列は非単調 (-101.5 → -6.7 → +32.8 → -3.2 → +47.2 → -18.6 → -30.9 → -7.2 Gi)。keys **786** (7 反続静止) / du **11403092 KB** KB 完全同値 (7 反続) / HEAD **c8090649** (7 反続)。resident PID 1083 継続。score 3 のまま (evidence/2026-09-20-falsify-123.md) |
| falsify-124 (2026-09-20): 「avail 大暴落 -47.6 Gi は dns-resolver annex 滞留再燃を反映」REFUTED — annex 静止系列が生存 (keys 786 / du 11403092 KB byte 同値 / HEAD c8090649 いずれも 8 反続、partial 14 件 2 法で連続実在 mtime Sep 6 不変)、帰属は du depth-1 タイムアウト中断で未確立 (数値主張なし); df avail 15738836 KB 15.0 GiB / 99%、緊急フラグ <100 Gi 3 反続発火、系列 -101.5 -> -6.7 -> +32.8 -> -3.2 -> +47.2 -> -18.6 -> -30.9 -> -7.2 -> -47.6 Gi 非単調継続; resident PID 1083 (9 反点)、export_and_sync 不在; score 3 (evidence/2026-09-20-falsify-124.md)
| falsify-125 (2026-09-21): 主張 (m)「falsify-124 比 avail -7.0 Gi の低下は dns-resolver annex 滞留再燃」を REFUTED — annex 静止系列が 9 反復連続で生存 (keys 786 / du 11403092 KB byte 同値 / HEAD c8090649 いずれも 9 反続、partial 14 件は total 365232 blocks / mtime Sep 6 19:12-19:27 で falsify-117..124 と完全同値、主張 (n)「partial 14 件 同一内容持続」は SURVIVED、Tier 2 dropunused 提案不変)。帰属は未確立: `du -x -d1 -m ~/github` が 180s timeout で中断 (falsify-124 の 420s timeout に続き 2 連続、du 出力未生成 — いかなる帰属数値も主張しない)。df avail **8545392 KB ≒ 8.1 GiB / 100%**、falsify-124 比 -7.0 Gi、緊急フラグ <100 Gi 4 反復連続発火、系列非単調継続 (-101.5 → -6.7 → +32.8 → -3.2 → +47.2 → -18.6 → -30.9 → -7.2 → -47.6 → -7.0 Gi、観測 122.3 → 103.75 → 69.8 → 62.6 → 15.0 → 8.1 GiB)。resident PID 1083 継続 (10 反復、Sep 18 15:52 起動)、export_and_sync 不在、git-annex プロセス不在。score 3 のまま (evidence/2026-09-21-falsify-125.md) |
| falsify-126 (2026-09-22): 主張 (o)「origin/main 前進 (679572b→a3dc8c9, 324 commits / 829 files / cljk rename wave) で falsify-63/75 の 2 既知 build-break が修理され現 tip スイートは load を通過する」を **REFUTED** — 既知 2 赤は修理済みを実測 (identity.cljk ns は (:import opener + 修飾ベクタ復元 行 29-34 / deps.edn が io.github.kotoba-lang/http を宣言 sha d54e2a23、修理経緯コメント付き) だが、isolated worktree (a3dc8c9) で `kbb -M:test` 2 回実行とも **決定論的 rc=1、新規 load 赤 authority.cljk:49 "Unable to resolve classname: java.nio.charset.StandardCharsets"** — ランタイムが JVM から nbb (Node v26.0.0) へ変化し JVM :import クラス解決が不可、加えて kbb 警告 100 deps not on classpath (GitHub deps が nbb に解決されない)。assertion 0 実行可能は継続 (failure site が identity.clj:1 → authority.cljk:49 へ変化)。**範囲修正**: tip ci.yml は default required path を JVM-free emit とし suite は leftover-jvm-tests.yml (workflow_dispatch 専用) へ降格 — 「コンパイル不能」は gate を赤にする現在形でなく leftover 手動 path のみ。src 256 / test 261 ファイルへ再アンカー (旧 244/247)。附帯: df avail 5.9Gi/100% 実測 (反復後半 11284096 KB ≒ 10.8 GiB で ±5 Gi 振動)、緊急フラグ <100 Gi **5 反続発火**、resident PID 1083 継続 (11 反点)、export_and_sync 不在。修理案 Tier 2: authority.cljk の JVM :import を reader 条件分離等で nbb 解決可能に + kbb 100 deps の nbb.edn 宣言判断。score 3 のまま (evidence/2026-09-22-falsify-126.md) |

| falsify-127 (2026-09-23, テスト 軸): 主張 (p)「falsify-126 の load 赤 authority.cljk:49 は一時的/環境起因で a3dc8c9 再実行では再現しない」を **REFUTED** — isolated worktree (/tmp/imwt127, a3dc8c9, 本体 touch なし) で kbb -M:test を 2 回実行、**両回とも決定論的 rc=1、同一 site** "Unable to resolve classname: java.nio.charset.StandardCharsets" (authority.cljk:49:3, nbb/Node v26.0.0) + kbb 警告 100 deps not on classpath が再現 — falsify-126 の「assertion 0 実行可能」は現在形として生存、赤は source 固有の決定論的 load 赤。score 3 のまま (実緑測定なし)。修理案 Tier 2 不変: authority.cljk (:import) を reader 条件分離等で nbb 解決可能に + kbb 100 deps の nbb.edn 宣言判断。附帯 (運用/滞留): df avail 5856912 KB ≒ 5.6 GiB / 100%、緊急フラグ <100 Gi 6 反続発火 (観測系列 103.75 → 69.8 → 62.6 → 15.0 → 8.1 → 5.6 GiB); dns-resolver annex 静止系列継続 (HEAD c8090649 10 反続 / keys 786 10 反続 / partial 14 件 ls 実在 2 法測定); resident PID 1083 継続 (12 反点)、export_and_sync 不在; 本体 porcelain clean、origin/main a3dc8c9 のまま。次の 1 アクション: authority.cljk 修理着地 (kanban/human) 後 suite load〜完走を再測定、未着地なら次反復は運用/滞留軸へ (evidence/2026-09-23-falsify-127.md) |

| falsify-128 (2026-09-23, 運用/滞留 軸): 主張 (q)「dns-resolver annex 静止系列 (keys 786 / du 11403092 KB / HEAD c8090649 / partial 14 件) は本反復で持続せず partial の一部が消滅する」を REFUTED 試行に対し **SURVIVED** — 2 法測定 (`git annex unused` 14 件列挙 rc=0 + `ls .git/annex/tmp` 14 ファイル実在 mtime Sep 6 19:12-19:27 不変、total 365232 blocks) で partial 14 件は連続実在、keys 786 / du 11403092 KB 完全同値 / HEAD c80906496 は **11 反続静止**、Tier 2 dropunused 提案 (14 件 ≈ 174 MiB) 不変再提出。附帯: df avail 8175048 KB ≒ 7.8 GiB / 100% (falsify-127 比 +2.2 Gi 微回復だが capacity 100% 標示継続)、緊急フラグ <100 Gi **7 反続発火** (観測系列 103.75 → 69.8 → 62.6 → 15.0 → 8.1 → 5.6 → 7.8 GiB 非単調、単一帰属未確立・帰属数値主張なし)、resident PID 1083 継続 (13 反点)、export_and_sync 不在 (ps grep -v grep 実測)。附帯 (テスト 軸・読み取りのみ): origin/main は a3dc8c9 → **414f907** へ前進 (2 commits、hermes_compat.cljk +8 行 contract のみ・PR #367)、authority.cljk は不触で falsify-126/127 の Tier 2 修理未着地 — suite load 赤の修理前提未充足。wt-msloop 本体 porcelain clean、測定のみで touch なし。score 3 のまま。次の 1 アクション: authority.cljk 修理着地を origin/main diff で確認し着地済みならテスト軸で suite load〜完走を再測定、未着地なら運用/滞留 軸で緊急フラグ 8 反続 / annex 静止系列 12 反続を判別 (evidence/2026-09-23-falsify-128.md) |

| falsify-129 (2026-09-24, テスト 軸): 主張 (r)「origin/main 前進 414f907→c4d3e08 で falsify-126/127 の Tier 2 修理 (authority.cljk nbb :import shim) が着地した」を **REFUTED** — 前進 2 commits (7023a68 rename + c4d3e08 merge) の diff は build.cljc→build.cljc.cljk (R100, 内容変更 0) + cljk-origin.edn (-520/+1) のみ、`git log 414f907..c4d3e08 -- '*authority*'` 0 行、tip c4d3e08 の authority.cljk:49 は bare JVM `(:import [java.nio.charset StandardCharsets])` が reader 条件分離なしで残存 (行 121 StandardCharsets/UTF_8 使用側も不変) — nbb 決定論的 load 赤 (authority.cljk:49, Node v26.0.0) の source 前提は不変で falsify-126/127 の「assertion 0 実行可能」は現在形生存、suite load〜完走の再測定前提は未充足。附帯: 本体 checkout 28105be [fix/ci-install-babashka] porcelain clean、origin/main 未包含 (merge-base --is-ancestor rc=1); wt-msloop HEAD 72f449b clean、origin/main 比 283 commits 遅行; **運用/滞留 軸は本反復未測定** (測定結合コマンドがセキュリティスキャンにより実行前に BLOCK、捏造ゼロ規約により数値記録なし — 緊急フラグ 8 反続 / annex 静止系列 12 反続の判定は次反復へ繰り越し)。score 3 のまま。次の 1 アクション: authority.cljk 修理着地を origin/main diff で再確認、未着地なら運用/滞留軸を単純コマンド分割で測定 (evidence/2026-09-24-falsify-129.md) |
