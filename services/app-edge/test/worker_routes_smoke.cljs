(ns worker-routes-smoke
  ...
  The fleet's size is READ from the catalog the Worker serves, not typed in
  here. Three checks used to name 1215 and 77, and a deploy that shipped a
  regenerated catalog (1294 / 79) turned them red without anything having
  broken — a test that fails when the world grows teaches its reader to ignore
  it. Reading `:count` and `:callable-count` keeps the claim that was actually
  wanted: the API and the generator agree about the same catalog.
...
(defn- catalog-counts
  "`:count` and `:callable-count`, from the catalog asset this Worker serves.

  An independent source: the generator wrote them when it built the file, so
  comparing the API against them is a parity claim and not a restatement of
  what the API just said."
  []
  (-> (js/fetch (str base "/itonami-fleet-catalog.edn"))
      (.then (fn [r]
               (when-not (.-ok r)
                 (throw (js/Error. (str "catalog asset unreadable: " (.-status r)))))
               (.text r)))
      (.then (fn [text]
               (let [c (edn/read-string text)]
                 (when-not (and (number? (:count c)) (number? (:callable-count c)))
                   (throw (js/Error. "catalog declares no :count / :callable-count")))
                 {:total (:count c) :callable (:callable-count c)})))))

(println "worker routes smoke —" base)

(-> (catalog-counts)
    (.then
     (fn [{:keys [total callable]}]
       (println "  catalog declares" total "actors," callable "callable")
       (-> (js/Promise.resolve)
    (.then #(check-route
    ...
    (.then #(check-route
             {:label "callable=true matches the catalog's own count"
              :path "/api/fleet/search?callable=true&limit=1" :expect-status 200
              :body-check (fn [b] (when-not (= callable (get b "matched"))
             ...
             {:label "an execution filter constrains, and every hit honours it"
              ...
              :body-check (fn [b]
                            (cond
                              ...
                              (>= (get b "matched") total) "execution filter matched everything"
    ...
    (.then #(check-route
             {:label "a blank parameter does not constrain"
              ...
              :body-check (fn [b] (when-not (= total (get b "matched"))
                                    (str "matched was " (get b "matched"))))}))
    (.then #(check-route
             {:label "limit pages the result but `matched` stays honest"
              ...
              :body-check (fn [b]
                            (cond
                              ...
                              (not= total (get b "matched")) "matched was paged too"))}))
    (.then #(check-route
    ...
    (.then (fn [_]
             (if (pos? @failures)
               ...
               (println "\nall routes behaved")))))))
    (.catch (fn [e]
              ;; The counts could not be read, so nothing below them was
              ;; checked. Say that, and exit non-zero: a smoke run that could
              ;; not start must not be mistaken for one that found nothing.
              (println "\nFAILED: could not read the catalog counts —" (str e))
              (js/process.exit 1))))
