  ...
  [:section.bw-capital
   ...
   (if deployed?
    [:div
     ...
     [:details.bw-capital-technical [:summary (l "ラウンド・取引履歴" "Rounds & transactions")]
      [:label.bw-funding-field (l "貸付ラウンド" "Lending round") [:select {:value (:vault capital) :on-change #((:capital-round handlers) (.. % -target -value))}
       (for [[i round] (map-indexed vector (:rounds capital))] [:option {:key (:vault round) :value (:vault round)} (str (l "ラウンド " "Round ") (inc i) " · " (short-address (:vault round)))])]]
   ...
   [:details.bw-capital-admin [:summary (l "運営者向け：募集・資金管理" "For operators: fundraising & management")]
    ...
    (when deployed? [:div
     [:label.bw-funding-field (l "管理する操作" "Management action") [:select {:aria-label (l "管理する操作" "Management action") :value admin-action :on-change #((:capital-field handlers) :admin-action (.. % -target -value))}
      (for [[id ja en] (drop 2 actions) :when (not= id "claim")] [:option {:key id :value id} (l ja en)])]]
     ...
     (when (= admin-action "repay") [:div (field :principal "返済する元本（USDC）" "Principal repayment (USDC)") (field :income "事業収益（USDC、なしは0）" "Income (USDC; 0 if none)")])
